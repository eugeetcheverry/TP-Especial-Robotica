...
initPose = [22; 22; -pi/2];           % Pose inicial (x y theta) del robot simulado (el robot puede arrancar en cualquier lugar valido del mapa)
...
for idx = 2:numel(tVec)   
   ...
   if mod(idx, 10) == 0 && not(i_found_myself)
    particles2 = initialize_particles(num_aux_particles, [free_y, free_x], map);
    weight2 = measurement_model(ranges, particles2, lidar2, sigma);
   end
   
   weight = measurement_model(ranges, new_particles, lidar2, sigma);

   if  max(weight2) > max(weight) && not(i_found_myself)
        indices = find(weight2 > max(weight));
        if 2*size(indices,1) > size(particles,1)
            indices = indices(1:floor(size(particles,1)/2));
        end
        [~, ind_min] = mink(weight, 2*size(indices,1));
        new_particles(ind_min,:) = [particles2(indices, :);particles2(indices, :)];
       ...
       if neff < 10
           particles = resample(new_particles, weight, ruido_resample);
   ...
   if cont_i_found_myself > 80 && not(i_found_myself)
        ...
        plan = plan(1:20:size(plan, 1), :);
        next_pos_index = 2;
        %ruido_resample = false;
        sigma = 0.5;
   end
   ...
   if not(i_found_myself) || very_close > 0
     %% Velocidades TimeStep
        ...
        if min_dist_center < 0.30 %Ya que el lidar esta mas adelante
            v_cmd = 0;
            w_cmd = 0.35;
        end
        %{
        if very_close > 0
            ...
            very_close = very_close - 1;
        end 
        %}
   else
       
       if pdist([pose_estimada(1:2);plan(next_pos_index, :)], 'euclidean') > 3 %Si me alejo mucho del plan, planeo de nuevo
            v_cmd = 0;
            w_cmd = 0;
            i = 0;
            for i = [1:100]
    ...
    set(h_particles, ...
        'XData', new_particles(:,1), ...
        'YData', new_particles(:,2));
    set(i_particles, ...
        'XData', particles2(:,1), ...
        'YData', particles2(:,2));
